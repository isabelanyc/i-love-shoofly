...
const routes = {
  about: () => `
    <section class="page about-page"><div class="about-grid">
      <div><h1 class="route-title">I love<br>shoofly pie.</h1></div>
      <div><p class="lead">This is an <a href="https://groups.etown.edu/amishstudies/frequently-asked-questions/" target="_blank" rel="noreferrer" title="Amish term for a non-Amish person">Englisher&rsquo;s</a> love letter to shoofly pie—a celebration of the molasses-rich treat and the Amish and Pennsylvania Dutch communities whose baking traditions made it a regional favorite.</p><p>I Love Shoofly is a public guide to shoofly pie—what it is, where it comes from, and where to find a slice. Browse the map, compare wet- and dry-bottom styles, explore the history, or suggest a place that should be included.</p><div class="about-actions"><a class="button button-primary" href="/where-to-find-shoofly-pie/">Find shoofly pie</a><a class="text-link" href="/what-is-shoofly-pie/">What is shoofly pie? ↗</a></div></div>
      <aside><span class="big-number">${spots.length}</span><span>shoofly pie stops found<br>and counting</span></aside>
    </div></section>`,
  what: () => `
  ...
  history: () => `
    <section class="page history-page">
      <div class="history-top"><div class="history-intro"><p class="section-kicker">A sticky history</p><h1 class="route-title">The history of shoofly pie.</h1><p>The honest history of shoofly pie is part record and part food lore. What survives points to a practical, eggless molasses bake shaped by Pennsylvania Dutch kitchens in the late nineteenth century.</p></div>
  ...
  contact: () => `
    <section class="page contact-page">
      <div class="contact-copy"><p class="section-kicker">Get in touch</p><h1 class="route-title">Let&rsquo;s talk<br>shoofly pie.</h1><p>Know a shoofly pie stop that belongs on the map? Have a correction, a shoofly pie story, a question, or an idea for working together? Send a note—every kind of shoofly pie message is welcome.</p><div class="contact-pie" aria-hidden="true">🥧</div></div>
      <form id="suggest-form"><label>What&rsquo;s this about?<select name="reason" required><option value="" selected disabled>Choose a reason</option><option>Suggest a shoofly pie stop</option><option>Update or correct a listing</option><option>Share a shoofly pie story</option><option>Press or partnership</option><option>Something else</option></select></label><div class="field-row"><label>Your name<input name="sender_name" required maxlength="120" autocomplete="name" placeholder="Your name"></label><label>Your email <span>(optional, if you&rsquo;d like a reply)</span><input name="email" type="email" maxlength="200" autocomplete="email" placeholder="you@example.com"></label></div><div class="field-row"><label>Spot or organization <span>(if applicable)</span><input name="spot_name" maxlength="120" placeholder="Bakery, market, or group"></label><label>Town / state <span>(optional)</span><input name="town" maxlength="100" placeholder="Lancaster, PA"></label></div><label>Google Maps or website link <span>(optional)</span><input name="url" type="url" maxlength="500" placeholder="https://…"></label><label>Your message<textarea name="message" required maxlength="1500" rows="4" placeholder="Tell us what&rsquo;s on your mind…"></textarea></label><div class="form-footer"><button class="button button-primary" type="submit">Send message</button><p id="form-status" role="status" aria-live="polite">Your message is sent privately by email.</p></div></form>
    </section>`
};
